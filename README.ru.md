...
Claude Code и без этого помнит между сессиями - чем отличается, в следующем разделе.
Чего он не делает, так это не гарантирует, что запись вообще случилась. Память пишет та же
модель, которая занята твоей задачей, по ходу дела, из еще не законченного разговора.
Сессию, где два часа все шло не туда и только в конце сошлось, он пересказывает хуже
всего - конец на момент записи еще не наступил.

Плюс сессий становится много. Объединить их нельзя, и никто не скажет, какие из них
остались неразобранными.

Что добавляется: после сессии она попадает в очередь, ты запускаешь `/sync-memory`, и
отдельный агент вычитывает переписку целиком, уже зная, чем все кончилось, и раскладывает
по файлам то, что выжило. Файлы Claude Code подхватывает при следующем старте в том же
репозитории.
...
В конце он предложит поставить в очередь твои последние содержательные сессии. Соглашайся:
на свежей установке очередь пуста, и первый `/sync-memory` разберет только что открытую
сессию, то есть не напишет ничего - выглядит это ровно как сломанный инструмент. С парой
реальных сессий в очереди первый прогон дает результат, который можно оценить. Делается
только при первой установке, обновление молча работу не досыпает.
...
`docs/GLOSSARY.md` (термины) и `docs/RECENT.md` (короткий лог сессий). Подстраивается под
конвенцию, которая в репозитории уже есть, коммитит поимённо только то, что написал сам, и
не пушит. Твои незакоммиченные правки не трогает.
...
и при старте сессии показывает строку с ее номером: "version 0.6.0 is out, you have 0.5.0".
Сам ничего не ставит.

```bash
bash install.sh --update
```

При обновлении выводится все, что вышло с той версии, которая у тебя стояла - прямо из
...
51 проверка, несколько секунд, без сети. Гоняются на одноразовых профилях
`CLAUDE_CONFIG_DIR` и локальных git-репозиториях, реальную установку не трогают. Можно
запустить одну группу: `bash test/run.sh queue`.

Покрыто то, что дорого сломать: merge `settings.json` не съедает чужие хуки, снятие убирает
только свое, все фильтры очереди, условия засева, резолв папки памяти для репозиториев и
worktree, проверка обновлений, пороги и антиспам сторожа контекста.
...
lib/memory-dir.sh          вычисляет, какой папке памяти соответствует директория
templates/MEMORY.md        затравка оглавления памяти
install.sh                 установка, обновление, удаление
test/run.sh                набор тестов
```
